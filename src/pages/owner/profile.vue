    ...
    <header class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
      <div>
        <nav class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">
          <NuxtLink to="/owner" class="hover:text-brand-blue transition-colors">Dashboard</NuxtLink>
        ...
        <p class="text-gray-400 font-bold uppercase tracking-widest text-xs mt-1">Customize how you're seen by the platform and attendees</p>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
      ...
      <div class="space-y-8">
        <div class="p-8 bg-white border border-gray-100 rounded-3xl shadow-sm text-center">
          ...
          <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ profileData.role.toUpperCase() }} ACCESS</p>
        </div>
           ...
           <div class="space-y-6">
              <div v-for="i in 2" :key="i" class="flex items-center justify-between py-4 border-b border-gray-50 last:border-0 opacity-40 grayscale group hover:opacity-100 hover:grayscale-0 transition-all">
                 <div>
                    ...
                    <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Receive email updates for active experiences</p>
                 </div>
...
const profileData = ref({
  name: 'Owen Owner',
  email: 'owen@eventhub.com',
  location: 'Downtown District, City Plaza',
  bio: 'Professional event curator with over 10 years of experience in organizing high-impact technology workshops and corporate networking experiences. Focused on delivering premium value to every attendee.',
  avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Owen',
  role: 'Owner'
});

const stats = [
  { label: 'Platform Tenure', value: '2.4 Years' },
  { label: 'Experience Rating', value: '4.92 / 5.0' },
  { label: 'Event Capacity', value: '98%' },
];
